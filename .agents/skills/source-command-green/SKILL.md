...
Use this skill when the user asks to run the migrated source command `green`.
...
Treat any user-provided text, feature directory, story path, or plan path as the command input. Use it to identify the target implementation area before making changes.
...
Invoke this skill as `$source-command-green`.
