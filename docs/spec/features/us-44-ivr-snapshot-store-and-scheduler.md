...
- **Collector picks up all active-position underlyings** — for positions in non-CLOSED phases, the collector calls `fetchIVR` for each distinct underlying, paced by the scraper's 1 req/s rate limiter. _(Superseded by [US-97](./us-97-collect-ivr-for-watchlist-underlyings.md): the batch is now the union of open positions and the watchlist.)_
...
- **Manual trigger from settings** — clicking "Refresh IVR now" runs the collector immediately for all collection targets and returns the batch summary (success/error counts) to the renderer. _(US-97 widened the targets to include the watchlist and disabled the button while a run is in flight.)_
...
The collector service `collectIVRSnapshots` lives in `src/main/services/ivr-collector.ts`. Its signature accepts `{ db, brokerProvider, logger, fetchIvr?, clock? }`, where `fetchIvr` and `clock` are optional injectable seams for tests (fake fetch / clock). It derives collection targets directly from SQLite — as shipped in US-44, distinct `ticker` values from `positions` where `status != 'CLOSED'`; since [US-97](./us-97-collect-ivr-for-watchlist-underlyings.md), the **union** of those and every `watchlist` row — normalized to uppercase, de-duplicated, and sorted alphabetically for deterministic batch order, keeping the collector independent of renderer list queries. It builds on `fetchIVR` from `src/main/integrations/barchart-ivr-scraper.ts` (see [US-43](./us-43-barchart-ivr-scraper.md)) and persists `source = 'barchart'`.

The 1 request/second politeness rule is enforced by the scraper module's internal rate limiter, which every `fetchIVR` call awaits; the collector's sequential loop adds no sleep of its own. _(US-44 originally double-throttled — a collector-level sleep on top of the scraper limiter — which US-97 review hardening removed.)_ Each scraper outcome is counted: an `ok` result is persisted and counts as a success; `not_available` writes no row, logs INFO, and counts as skipped; `parse_error`/`network_error`/`rate_limited`/`invalid_input` count as errors, log WARN, and the batch continues. _(US-97 added a per-ticker `try/catch` so a fetch that **throws** is isolated the same way; a `persistSnapshot` throw is systemic and aborts the run — see [ADR](../architecture/02-adrs/ivr-collector-per-ticker-failure-isolation.md).)_ Persistence applies same-day overwrite semantics: inside a transaction it deletes any existing row for the same `(underlying, UTC-calendar-date(observed_at))` before inserting the fresh row, so the latest same-day value wins despite the `(underlying, observed_at)` primary key.

A non-trading-day guard sits at the top of `collectIVRSnapshots`. When `BrokerProvider.getMarketStatus()` reports a closed, non-trading day it returns an `ok` batch summary of `successCount=0, errorCount=0, skippedCount=0, skippedReason='market_closed'` before any fetch. Because the guard lives in the collector, both the scheduled path and the manual trigger share one code path and one logging decision. If the status read fails — or no broker is configured at all — the run logs and assumes a trading day rather than aborting: Barchart needs no broker, so a watchlist-only trader still gets collection.
...
- **Scraper-level throttling boundary** — the 1 req/sec rule is enforced by the scraper module's rate limiter, which every caller awaits; the collector adds no second sleep (its original extra sleep made per-ticker cost `1s + latency` and was removed in US-97 review hardening).
