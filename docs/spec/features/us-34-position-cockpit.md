...
- **AC-7 Collapsible drawers** — Two drawers under the risk / context cards: "Leg reference" (5 or 6 fields depending on whether a snapshot is present) and "Cost basis & history" (2 stats + optional `LegHistoryTable`). Header: chevron (▶ closed / ▼ open) + title + "N fields" right-label. Collapsed by default when an active leg exists; "Cost basis & history" is `defaultOpen` when there is no active leg. Implemented with a self-managed `useState` toggle on a `<button>` carrying `aria-expanded`.
...
- **`CollapsedDrawer`** — self-contained `useState`-toggled drawer (imports only `{ useState, type ReactNode }` from React) with a chevron header (▶/▼), title, "N fields" right label, and a `<button>` carrying `aria-expanded`.
...
- **Self-managed `useState` drawer toggle** — `CollapsedDrawer` rolls its own open/closed state with `useState` on a `<button>` that sets `aria-expanded`, rather than pulling in a shadcn `Collapsible` primitive. No `ui/collapsible.tsx` was added → [[useState-collapsible-drawers]]
