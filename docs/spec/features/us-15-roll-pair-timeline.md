...
Color tinting (credit background for header rows, blue-tint for roll legs, cumulative-row background) is centralized in module-level constants in `LegHistoryTable.tsx` (`ROLL_CREDIT_BG`, `ROLL_LEG_BG`, `CUMULATIVE_BG`). Inline `style` is reserved for the cases where colors are runtime-derived from `rollCreditDebitColors` per group.
...
- **Module-level color constants over inline magic rgba strings.** `ROLL_CREDIT_BG`, `ROLL_LEG_BG`, `CUMULATIVE_BG` (defined in `LegHistoryTable.tsx`) are single-source-of-truth values; runtime-derived per-group colors (including the debit background) remain inline because they depend on `rollCreditDebitColors`.
...
- [src/renderer/src/components/LegHistoryTable.tsx](../../../src/renderer/src/components/LegHistoryTable.tsx) — `TimelineItem` dispatch; `LegRow` / `RollGroupHeaderRow` / `CumulativeSummaryRow` internal components; module-level color constants (`ROLL_CREDIT_BG`, `ROLL_LEG_BG`, `CUMULATIVE_BG`)
- [src/renderer/src/lib/rollGroups.ts](../../../src/renderer/src/lib/rollGroups.ts) — new module: `LegHistoryEntry`, `TimelineItem`, `buildRollTimeline`, `computeCumulativeRollSummary`
