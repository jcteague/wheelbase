...
This is a **broker** concern. Market clock/session derivation lives on the `BrokerProvider` interface (served by `AlpacaBrokerProvider` in `src/main/integrations/alpaca-broker.ts`) on the `broker:market-status` IPC channel. The Alpaca→Massive migration split account, market-status, and activities off the `MarketDataProvider` interface onto a dedicated `broker:*` namespace — the broker stays Alpaca while market data moved to Massive. There is **no** `market-data:market-status` channel.
...
- **Keep market status on `MarketDataProvider` / a `market-data:market-status` channel** — rejected by the Massive migration; market data and broker are separate vendors, so status moved to `broker:market-status`.
