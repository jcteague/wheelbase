...
<!-- generated:from us-2,us-4,us-5,us-6,us-7,us-8,us-8-pct-fix,us-9,us-10,us-11,us-12,us-12-refactor,us-13,us-14,us-15,us-16,us-17,us-31,us-32,us-33,us-34,us-35,us-37,us-39,us-43,us-44,us-50,us-51,us-52,us-53-54-55,us-56,us-57-58,us-59,us-60,us-62,us-63,us-64,us-65,us-66,us-67,us-68,us-70,us-97,us-98,us-99,market-data-massive-migration,missing-ac,design-system,extract-sheet-primitives,fix-sheet-portal-styles,frontend-perf-reuse -->
...
- [Market Data](domain/market-data.md) — live underlying prices, option snapshots/chains, stream-first transport, market-status pill, and the `MarketDataProvider` adapter (Alpaca free data plan since US-99)
...
- [Alpaca Integration](contracts/alpaca-integration.md) — broker SDK isolation, the raw-`fetch`/`ws` market-data adapter, shared credentials and the credential-change restart, error handling
...
- [Market-Data Provider Migration: Alpaca → Massive](features/market-data-massive-migration.md) — **superseded by US-99.** Retro plan capturing the interim swap of the live market-data provider from Alpaca to Massive (provider-agnostic interface + `marketDataFactory` + `MassiveMarketDataProvider`; broker concerns split onto `broker:*`); the interface, factory and broker split it records are still current, the Massive vendor surface is not
- [US-99 — Alpaca as the sole market-data provider](features/us-99-alpaca-market-data-provider.md) — retires Massive: `AlpacaMarketDataProvider` serves the whole `MarketDataProvider` interface from Alpaca's **free** data plan (one batched IEX stock snapshot for REST seeds, an IEX websocket with per-symbol `bars` subscriptions, indicative option chains/snapshots with real bid/ask + greeks, open interest joined from the trading API's contracts endpoint) using the trader's saved Alpaca broker credentials, resolved on every call. The factory never throws; a credential save/remove/switch restarts only the stock stream. `CredentialStatus.marketData` replaces `massive`, `settings:test-connection` is Alpaca-only, and Settings/Positions/Screener copy names Alpaca. No schema or IPC-channel change; `grep -rni massive src e2e .env.example` prints nothing
...
- [US-64 — Pull option chains for watchlist tickers](features/us-64-pull-option-chains-for-watchlist.md) — the screener's data-acquisition layer: `pullWatchlistChains` fetches the put side of each watchlist ticker's chain within a DTE window (default 30–45) through the `MarketDataProvider` adapter, returning per-strike bid/ask/mark/delta/OI/volume + quote timestamp. Widens the adapter's chain return to `OptionChainQuote` (per-strike identity + real OI/volume), adds the pure `candidate-chain` helpers, and runs a concurrency-bounded, failure-isolated fan-out that distinguishes `provider_unavailable` / `no_options_listed` / per-ticker `data_unavailable` from a genuine zero-candidate result. Backend-only — scoring is US-65, display is US-66
- [US-65 — Score wheel candidates against configurable screening criteria](features/us-65-score-wheel-candidates.md) — turns US-64's raw chains into a ranked, explainable candidate list: a pure `core/screener.ts` engine disqualifies strikes against seven ordered hard filters (price ceiling, earnings, DTE, delta availability, delta band, open interest, spread — first failure wins, each with a verbatim-rendered reason), scores survivors on premium yield, and ranks one strike per ticker by yield-per-delta (annualized return-if-flat ÷ |delta|). Adds the first read path over `ivr_snapshot` (`getLatestIvrByUnderlying`, IVR as a soft rank-only input), a boundary-isolated `screenWatchlistCandidates` service, and the payload-free `screener:results` IPC channel. No migration, no renderer surface — the table is US-66, the criteria UI is US-67, earnings dates are US-70
- [US-66 — Display ranked screener results with key metrics](features/us-66-screener-results.md) — the epic's payoff surface: a `/screener` page rendering US-65's scored candidates as a dense 12-column table in the engine's yield-per-delta order (the renderer never re-sorts), with a collapsed `Excluded (n)` section showing every non-ranking ticker's verbatim reason so the list is visibly complete. Three visually distinct body states — ranked / "no candidates match your criteria" / "market data unavailable" (a status-preserving adapter keeps `provider_unavailable` as data, not an error) — plus a `Stale snapshot` badge and quote-time caption when the `MarketStatusPill` reads CLOSED. Adds seven pure display formatters, the `screener:results` adapter + query hook, and the `⌕ Screener` nav item; no IPC, service, engine, or migration. Promote (US-68), criteria editing (US-67), and the earnings badge (US-70) are out of scope
- [US-67 — Configure screening criteria](features/us-67-configure-screening-criteria.md) — makes the screener's criteria editable from a 460px right-hand sheet on the Screener page itself (never Settings), persisted as one JSON document under the `app_settings` key `screening_criteria` and resolved by the service on every screen, closing US-65's unused `opts.criteria` seam. Adds the engine's missing `iv_rank_floor` filter — positioned immediately after `price_ceiling`, inclusive, and never excluding an unknown IV rank — plus a pure `core/screening-criteria.ts` bounds module imported by all three validation layers, the `screener:get-criteria` / `screener:save-criteria` channels, a clickable criteria summary strip, and three entry points into one sheet. Saving re-screens by invalidating both the criteria and results queries. No migration. Earnings handling is persisted here but applied in US-70
- [US-68 — Promote a screener result to the new-wheel form](features/us-68-promote-result-to-new-wheel.md) — the bridge from "best candidate" to an open wheel: a **Promote to trade** action on every ranked row opens the US-1 new-wheel form pre-filled with ticker, strike, expiration, premium and `contracts: 1`, plus a quote-provenance stamp and a `thesis` seeded from the ticker's watchlist note. The pre-filled premium is an editable default, never a lock — the form re-fetches a one-shot fresh quote for that exact contract and reconciles it into exactly one non-blocking banner (`offline > stale > moved > edited > match`), warning only when the mark moved past `max($0.05, 5%)`, and flagging CLOSED/EXT marks as stale after-hours snapshots with session-accurate copy. The re-fetch never writes into form state, every fetch failure degrades to a usable form, and the promoted payload is consumed once from the hash query string so it cannot resurrect on a later plain visit. Adds a live capital/yield-if-flat derived row; no IPC, service, engine, or migration

- [US-70 — Warn when a candidate has earnings within the DTE window](features/us-70-earnings-in-window-warning.md) — closes the screener's last silent pass: a candidate whose next earnings print lands on or before expiry is hard-excluded by default or, under `earningsHandling: 'flag'`, kept with a gold `⚠ Earnings` badge and sorted below every clean candidate. Widens US-56's shared Finnhub feed (caller-supplied `lookaheadDays`, and a four-state `EarningsLookup` that finally distinguishes "read the calendar, no event" from "could not read it"), replaces that feed's process-local cache with the persisted `earnings_date` table (migration 013) shared by the alert scheduler and the screener, and makes earnings certainty the outer ranking key in both `rankCandidates` and `screenTicker`. An unknown or unreachable date cautions and demotes but never excludes — failing closed on the vendor would silently empty the results table. Replaces `ScoredCandidate.earningsFlagged` with `earnings: CandidateEarnings` (breaking). BMO/AMC timing is out of scope
- [US-97 — Collect IVR snapshots for watchlist underlyings](features/us-97-collect-ivr-for-watchlist-underlyings.md) — widens the nightly IVR collection targets from open positions only to the **union** of open positions and the watchlist, so IV rank is populated for the bench names the screener actually ranks instead of reading `n/a` on exactly the rows the trader is evaluating. One SQL statement (`UNION`, with the existing uppercase/`Set`/sort pipeline guaranteeing a held-and-watchlisted ticker is fetched once) supersedes the US-44 "targets come from active positions" ADR; everything downstream — rate limiting, same-day overwrite, `ivr:collect-now`, the scheduler registration — was already target-agnostic. Also adds the per-ticker `try/catch` the batch-job isolation rule requires (a non-JSON scraper response _throws_ rather than returning a status, and a rejected handler aborts the run, so one bad ticker used to lose every ticker after it) and a pending state on "Refresh IVR now", now that the run grows ~1s per watchlist name. Consequence: US-67's IV-rank floor can finally exclude a thin-IVR bench name. No migration
- [US-98 — Age an IV-rank reading so a stale one can't pass as current](features/us-98-ivr-staleness-tiers.md) — **in progress.** Ages every IV rank against **completed exchange sessions** rather than calendar days, so Friday's close stays current on Monday and a holiday ages nothing, and against any earnings print known to have landed since — which invalidates a reading however new it is. The verdict (`fresh` 0–1 / `aging` 2–3 / `stale` 4–10 / expired) is computed once at the service boundary and shared by every surface: only fresh and aging readings reach `screenTicker`, so `iv_rank_floor` never fires on data the app cannot vouch for, while the muted value and its age still reach the trader through the new shared `IvrCell`. Adds `trading_session` (migration 015), a cache of the venue's own calendar fetched through the new `BrokerProvider.getMarketCalendar` — closures stored explicitly so an unfetched day reads as _unknown_, never as closed — which also replaces the collector's broker-clock holiday guard. Splits earnings history from the next print (`earnings_date.last_earnings`, migration 014) behind one shared resolver, leaving US-70's callers untouched. **11 of 13 ACs ship and are verified end to end**; AC5 and AC13 wait on US-96 for the watchlist Signal, and the tier boundaries still await trader validation
