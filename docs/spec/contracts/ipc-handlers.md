...
- **[US-116] Canonical.** The exchange session is a fact about the market, not about the trader's account, so it is served here rather than on a `broker:` channel. `broker:market-status` is **removed**. Registered in `src/main/ipc/market-data.ts`, served by `AlpacaMarketDataProvider.getMarketStatus()` from `{tradingBase}/v2/clock`.
- **Purpose:** return current session (`regular`/`pre`/`post`/`closed`) plus `nextOpen`/`nextClose` timestamps. Polled by `useMarketStatus()` every 60 s to drive the `MarketStatusPill`. Reached from the renderer as `window.api.marketData.marketStatus()`, keyed `marketDataQueryKeys.marketStatus = ['market', 'status']`.
- **Request:** none.
- **Response (success):**
  ```typescript
  {
...
- **Notes:** no payload, so no Zod schema — one `handleIpcCall` around a single `getProvider().getMarketStatus()`, thin per the IPC rule.
- **Source:** `src/main/ipc/market-data.ts`, `src/main/integrations/alpaca-market-data.ts`
- **Driven by:** [us-32 — Live Position Prices](../features/us-32-live-position-prices.md), [us-116 — Market facts from the market-data provider](../features/us-116-market-facts-from-market-data-provider.md)
...
- **[US-116] Removed.** The channel, its preload binding (`broker.marketStatus`), its result type
  (`IpcGetBrokerMarketStatusResult`) and the renderer's `api/broker.ts` `getMarketStatus` are all
  deleted. `BrokerProvider` is now exactly `getAccountInfo` + `getActivities`.
- **Replaced by:** [`market-data:market-status`](#market-datamarket-status), which returns the
  identical `{ ok: true, status }` payload from `AlpacaMarketDataProvider`. Only the channel, the
  service behind it and the preload path changed.
- **Why:** it gated a market fact on an optional broker relationship. With it on the broker
  namespace the market-status pill never resolved on a journal-only install.
- **Driven by:** [us-116 — Market facts from the market-data provider](../features/us-116-market-facts-from-market-data-provider.md)
...
| `auth_failed`                 | upstream market-data or broker provider rejected credentials                                                                               | `market-data:stock-quotes`, `market-data:set-stock-quote-tickers`, `market-data:market-status`, `market-data:option-snapshot`, `market-data:option-chain`, `broker:account`, `broker:activities`                       |
| `network_error`               | upstream provider unreachable                                                                                                              | `market-data:stock-quotes`, `market-data:set-stock-quote-tickers`, `market-data:market-status`, `market-data:option-snapshot`, `market-data:option-chain`, `broker:account`, `broker:activities`                       |
| `rate_limited`                | upstream provider returned 429                                                                                                             | `market-data:stock-quotes`, `market-data:set-stock-quote-tickers`, `market-data:market-status`, `market-data:option-snapshot`, `market-data:option-chain`                                                              |
