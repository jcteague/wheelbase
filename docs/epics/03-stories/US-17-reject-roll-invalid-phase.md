...
```gherkin
Scenario Outline: Roll rejected for non-rollable phases
  Given the position is in phase "<phase>"
  When the trader attempts to roll the position
  Then the roll is rejected with message "Cannot roll: position is in <phase> phase. Only CSP_OPEN and CC_OPEN positions can be rolled."
...
Scenario: CSP roll rejected when position is CC_OPEN
  Given the position is in phase CC_OPEN
  When a CSP roll is attempted via IPC
  Then the roll is rejected with message "Cannot roll CSP: position is in CC_OPEN phase, not CSP_OPEN"

Scenario: CC roll rejected when position is CSP_OPEN
  Given the position is in phase CSP_OPEN
  When a CC roll is attempted via IPC
  Then the roll is rejected with message "Cannot roll CC: position is in CSP_OPEN phase, not CC_OPEN"
...
Scenario: Roll rejected after CSP has already been closed
  ...
  Then the roll is rejected
  And the error message explains the position is no longer in a rollable state
```
...
- **Lifecycle engine:** Both `rollCsp` and `rollCc` (from US-12/14) validate the current phase as their first check. This story ensures comprehensive test coverage for all non-rollable phases, not new implementation.
- **IPC layer:** The error response follows existing pattern: `{ ok: false, errors: [{ field: 'phase', code: 'INVALID_PHASE', message: '...' }] }`.
